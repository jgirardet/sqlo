...
async fn main() {
    ...
    .await.unwrap();
    //test tablname
    assert_eq!(Maison::tablename(), "maison", "tablename is derived from struct name");
    assert_eq!(WithAttrs::tablename(), "piece", "attribute tablename force override tablename");
    ...
    // test column rename
    assert_eq!(p,WithAttrs {nb:uu4!(1), lgogog:1, la:10, maison_id:1});

    // test create : primary_key auto generated by db
    // test create returning option and non option
    let nm = Maison::create(&mut conn, "labas".to_string(), 23, None).await.unwrap();
    ...
    let nm2 = Maison2::create(&mut conn, 999, "le999".to_string(), 9, None).await.unwrap();
    ...
    let mut sn = Maison {id: 123,adresse:"zef".to_string(),taille: 234,piscine: Some(true)};
    ...
    sn.save(&mut conn).await.unwrap();
    assert_eq!(Maison::get(&mut conn, 123).await.unwrap().adresse, "AA".to_string());
